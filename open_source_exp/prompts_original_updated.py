...
'''
prompt_perils = '''Based on the provided article {info_box} {Whole_text}, please extract information about the perils {Event_Name} , and assign the details as follows:
...
prompts = {
    ...
    "prompt_perils": prompt_perils,
    "prompt_location": prompt_location,
    "prompt_country": prompt_country,
    "prompt_time": prompt_time,
    "prompt_death_per_country": prompt_death_per_country,
    "prompt_injuries_per_country": prompt_injuries_per_country,
    "prompt_displaced_per_country": prompt_displaced_per_country,
    "prompt_homeless_per_country": prompt_homeless_per_country,
    "prompt_affected_per_country": prompt_affected_per_country,
    "prompt_insure_per_country": prompt_insure_per_country,
    "prompt_total_per_country": prompt_total_per_country,
    "prompt_building_per_country": prompt_building_per_country
}

# Instruction to be appended to each prompt
instruction = "\nEnsure the output is STRICTLY in JSON format. Use ONLY the provided field names and DO NOT modify, add, or create new field names. " \
              "The JSON should contain NO metadata, comments, or explanatory text outside the defined fields. Maintain the structure as described, STRICTLY " \
              "adhering to the order and nesting of the elements. Output MUST NOT include any element not explicitly defined in the prompt."

# Loop through the dictionary and append the instruction to each prompt
for key, value in prompts.items():
    prompts[key] = value.strip() + instruction