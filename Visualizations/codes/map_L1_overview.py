...
def count_events_per_admin_area(df):
    ...
    event_count_per_admin_area = df_exploded.groupby("Administrative_Areas")["Main_Event"].count().reset_index()
...
def plot_main_events_per_admin_area(df, geojson_file, country, event_count, output_file):
    ...
    ax.set_xticks([])
    ax.set_yticks([])
    plt.title(f"Spatial distribution of main events in Wikipmacts 1.0 database")
