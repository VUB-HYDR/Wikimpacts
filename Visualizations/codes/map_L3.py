...
def count_events_per_admin_area(df):
    ...
    event_count_per_admin_area = df_exploded.groupby("Locations").size().reset_index(name="Count")
...
def plot_impacts_per_location(df, geojson_file, state, event_count, output_file):
    """
    ...
    for gj_list in df["Locations_GeoJson"]:
    ...
    gdf = gpd.GeoDataFrame({"Location": area_names, "geometry": geometries})
    ...
    ax.set_xticks([])
    ax.set_yticks([])
    plt.title(f"Spatial distribution of impact data points in sub-national scale in Wikimpacts 1.0 database")
