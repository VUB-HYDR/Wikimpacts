...
from Evaluation_V2.matcher import CurrencyMatcher, SpecificInstanceMatcher
from Evaluation_V2.utils import Logging
from Evaluation_V2.weights import weights as weights_dict
...
if __name__ == "__main__":
    ...
    if args.event_level == "l1":
        ...
        if "insured" in args.impact_type.lower():
            monetary_categories = ["Insured_Damage"]
        elif "damage" in args.impact_type.lower() and not "insured" in args.impact_type.lower() and not "buildings" in args.impact_type.lower():
    ...
    for mc in monetary_categories:
        ...
        for col in [unit_col, adjusted_col, year_col, min_col, max_col]:
            gold[col] = gold.apply(
                lambda row: (
                    row[col][row[aliged_col]]
                    if row[aliged_col] >= 0
                    and row[col] is not None
                    and isinstance(row[col], list)
                    and len(row[col]) > row[aliged_col]
                    else None
                ),
                axis=1,
            )
