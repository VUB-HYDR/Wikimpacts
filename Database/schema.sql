CREATE TABLE Events (
  ...
  Single_Date_Day INTEGER CHECK (length(Single_Date_Day) <= 31),
  Single_Date_Month INTEGER CHECK (length(Single_Date_Month) <= 12),
  Single_Date_Year INTEGER CHECK (length(Single_Date_Year) = 4),
  Start_Date_Day INTEGER CHECK (length(Start_Date_Day) <= 31),
  Start_Date_Month INTEGER CHECK (length(Start_Date_Month) <= 12),
  Start_Date_Year INTEGER CHECK (length(Start_Date_Year) = 4),
  End_Date_Day INTEGER CHECK (length(End_Date_Day) <= 31),
  End_Date_Month INTEGER CHECK (length(End_Date_Month) <= 12),
  ...
  Total_Insured_Damage_Inflation_Adjusted_Year INTEGER CHECK (length(Total_Insured_Damage_Inflation_Adjusted_Year) = 4),
  Total_Damage REAL,
...
CREATE TABLE Specific_Instance_Per_Country_Death (
  ...
  Time_Death_Day INTEGER CHECK (length(Time_Death_Day) <= 31),
  Time_Death_Month INTEGER CHECK (length(Time_Death_Month) <= 12),
...
CREATE TABLE Specific_Instance_Per_Country_Injuries (
  Event_ID TEXT NOT NULL CHECK (length(Event_ID)),  /* COMMENT 'UID', */
  Country TEXT NOT NULL,  /* COMMENT 'Country Code', */
  Location_Injuries TEXT,
  Location_GCS OBJECT,  /* COMMENT 'GCS', */
  Time_Injuries_Day INTEGER CHECK (length(Time_Injuries_Day) <= 31),
  Time_Injuries_Month INTEGER CHECK (length(Time_Injuries_Month) <= 12),
...
CREATE TABLE Specific_Instance_Per_Country_Displacement (
  ...
  Location_GCS OBJECT,  /* COMMENT 'GCS', */
  Time_Displaced_Day INTEGER CHECK (length(Time_Displaced_Day) <= 31),
  Time_Displaced_Month INTEGER CHECK (length(Time_Displaced_Month) <= 12),
...
CREATE TABLE Specific_Instance_Per_Country_Homelessness (
  ...
  Location_GCS OBJECT,  /* COMMENT 'GCS', */
  Time_Homeless_Day INTEGER CHECK (length(Time_Homeless_Day) <= 31),
  Time_Homeless_Month INTEGER CHECK (length(Time_Homeless_Month) <= 12),
...
CREATE TABLE Specific_Instance_Per_Country_Insured_Damage (
  ...
  Insured_Damage_Inflation_Adjusted_Year INTEGER CHECK (length(Insured_Damage_Inflation_Adjusted_Year) = 4),
  FOREIGN KEY(Event_ID) REFERENCES Events(Event_ID)
);
...
CREATE TABLE Specific_Instance_Per_Country_Building_Damage (
  ...
  Location_GCS OBJECT,  /* COMMENT 'GCS', */
  Time_Building_Day INTEGER CHECK (length(Time_Building_Day) <= 31),
  Time_Building_Month INTEGER CHECK (length(Time_Building_Month) <= 12),
